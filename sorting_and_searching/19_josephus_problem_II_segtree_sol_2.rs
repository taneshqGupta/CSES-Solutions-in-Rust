...
impl SegTree {
    ...
    fn _range_sum(&self, v: usize, tl: usize, tr: usize, l: usize, r: usize) -> usize {
        if tl == l && tr == r {
            return self.t[v];
        }
        let tm = tl + (tr - tl) / 2;
        if r <= tm {
            return self._range_sum(v * 2, tl, tm, l, r);
        }
        if l > tm {
            return self._range_sum(v * 2 + 1, tm + 1, tr, l, r);
        }
        self._range_sum(v * 2, tl, tm, l, tm) + self._range_sum(v * 2 + 1, tm + 1, tr, tm + 1, r)
    }
    fn find_where_sum_is_k(&self, v: usize, tl: usize, tr: usize, k: usize) -> usize {
        if tl == tr {
            return tl;
        }
        let sum_in_left_subtree = self.t[v * 2];
        let tm = tl + (tr - tl) / 2;
        if sum_in_left_subtree < k {
            return self.find_where_sum_is_k(v * 2 + 1, tm + 1, tr, k - sum_in_left_subtree);
        }
        else {
            return self.find_where_sum_is_k(v * 2, tl, tm, k);
        }
    }
...
fn main() {
    ...
    while ring.t[1] > 0 {
        ...
        let target = curr + 1;
        let ans = ring.find_where_sum_is_k(1, 0, n - 1, target);
        print!("{} ", ans + 1);
